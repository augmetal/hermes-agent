...
# fcntl is POSIX-only; on Windows we use msvcrt.locking. Both expose
# _lock_exclusive(fd) / _unlock(fd) with the same semantics: an advisory
# exclusive lock on the file descriptor that is released on close.
# ---------------------------------------------------------------------------
if sys.platform == "win32":
    import msvcrt

    def _lock_exclusive(fd: int) -> None:
        # msvcrt.locking locks `nbytes` starting at the current file pointer.
        # Lock 1 byte at offset 0; LK_LOCK blocks (10 retries x 1s, then OSError).
        try:
...
class MemoryStore:
    ...
    def _file_lock(path: Path):
        ...
        fd = open(lock_path, "wb+")  # binary mode required for msvcrt.locking
        try:
            _lock_exclusive(fd.fileno())
            yield
        finally:
            _unlock(fd.fileno())
            fd.close()
